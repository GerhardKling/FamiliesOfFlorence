...
def start_button_function():
    ...
    #CHECK
    print(f"{player.name} belongs to the family {family.name}.")
...
def save_game():
    ...
    #CHECK
    print(f"SAVED: {player.name} belongs to the family {family.name}.")

def load_game():
    """Loads objects; default game file"""
    with open('game.pkl', 'rb') as input:
        obj = pickle.load(input)
    global player, family
    player = obj[0]
    family = obj[1]

    #CHECK
    print(f"LOADED: {player.name} belongs to the family {family.name}.")
